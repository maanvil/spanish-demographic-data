...
def generate_map(df, year, sex, prov, perMil):
    ...
    coropletas = folium.Choropleth(
        ...
        line_opacity=1.0,
        control=False,
        legend_name=f'Nacimientos ({"‰" if perMil else "cantidad"})'
    )

    for feature in coropletas.geojson.data['features']:
        code = feature['properties']['codigo']
        feature['properties']['Provincia'] = prov_dict[code]
        feature['properties']['NacimientosPorMil'] = f'{df[df.Codigo==code].NacimientosPorMil.values[0]:.2f} ‰'
        feature['properties']['Nacimientos'] = f'{df[df.Codigo==code].Nacimientos.values[0]:,} bebés'
...
st.write(
    '''En esta sección, encontrarás información detallada sobre los 
    nacimientos en España. Explora datos por sexo, año y provincia, 
    y descubre cómo ha evolucionado el número de nacimientos a lo largo del tiempo.''')

st.write(
    '''Sumérgete en las estadísticas demográficas y observa los patrones 
    y tendencias en los nacimientos. Además, disfruta de la visualización 
    en mapas de coropletas que te ayudarán a comprender las diferencias 
    geográficas en los nacimientos en cada provincia.'''
)
st.write('')
...
fig.update_traces(hovertemplate='%{y:.2f} ‰' if perMil else '%{y:,} bebés')
fig.update_layout(hovermode='closest', showlegend=(sex=='Total'), yaxis=dict(title='Nacimientos' + (' (‰)' if perMil else '')))
...
fig.add_annotation(
    x=year, y=value, text=f'{value:.2f} ‰' if perMil else f'{value:,} bebés', ax=0, ay=-75,
